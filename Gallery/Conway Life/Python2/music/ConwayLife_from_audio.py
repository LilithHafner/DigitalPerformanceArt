...
margin = 0
video = 0#np.inf
frame_rate = 60 if video else 18
ticks_per_frame = 3
depth = int(timespan*frame_rate)#int(np.log(255)*timespan)
sample_rate, samples = wavfile.read('mishma.wav')
frequencies, times, spectrogram = signal.spectrogram(samples, sample_rate, nperseg=1024)
spectrogram_frame_rate = spectrogram.shape[1]/(samples.size/sample_rate)
spectrogram *= frequencies.reshape(frequencies.shape+(1,))**2
def cadence(t):
##    out = np.zeros((width+height)*2-4)
    out = np.zeros(width)
    n = 2
    frame = int(t*spectrogram_frame_rate)
    if frame < spectrogram.shape[1]:
        for i in range(n):
             out[i::n] = spectrogram[:width//n,int(t*spectrogram_frame_rate)]//157695898
##            out[-width+i::n] = spectrogram[:width//n,int(t*spectrogram_frame_rate)] > 457695898
    return out
...
def tick(world, light, out=None):
    ...
    out[boarder(world.shape)] = 0#.5 < light#\
        #np.random.rand(len(boarder(world.shape)[0])) < light
    for i in range(1,height+1):
        out[height-i,:] |= light >= i
...
try:
    while True:
        frame = world.tick(cadence(sim_time), ticks_per_frame)[slc, slc]*255
        red = np.maximum(-frame,0)
        blue = np.maximum(frame,0)
        green = np.zeros(frame.shape)
        if not (w.write(np.array([blue, green, red]).transpose(1,2,0)) == -1 \
                or (video and sim_time < video)):
            break
        sim_time += 1/frame_rate
